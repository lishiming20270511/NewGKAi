...
# 超高分段保底分差校验阈值（TASK-15）
ULTRA_HIGH_SCORE_THRESHOLD = 670   # 超高分段起始线（软拦截）
ULTRA_HIGH_SCORE_FLOOR     = 680   # 硬拦截起始线
ULTRA_HIGH_BOTTOM_GAP      = 50    # 超高分段保底最大分差
ULTRA_HIGH_HARD_MIN_SCORE  = 600   # 硬拦截：保底院校历史最低录取分下限

# ──────────────────────────────────────────────────────────────────────────────
# 专业意向过滤（BUG-019）：院校名称关键词 → 院校专业类型
# ──────────────────────────────────────────────────────────────────────────────

# 仅匹配"单一专业性"院校（综合类、985/211综合大学不含任何关键词，不会被错误分类）
_SPECIALTY_SCHOOL_MARKERS: dict[str, list[str]] = {
    "艺术类": [
        "美术学院", "音乐学院", "舞蹈学院", "艺术学院", "戏剧学院",
        "影视学院", "电影学院", "演艺学院", "歌剧学院",
    ],
    "体育类": ["体育大学", "体育学院"],
    "政法类": ["政法大学", "政法学院", "公安大学", "警察学院", "司法学院"],
    "中医药类": ["中医药大学", "中医学院", "中医药学院"],
    "外语类": ["外国语大学", "外语学院", "语言大学", "国际关系学院"],
    "师范类": ["师范大学", "师范学院"],
    "财经类": ["财经大学", "财贸大学", "财经学院", "经贸大学", "贸易大学"],
    "农林类": ["农业大学", "农林大学", "林业大学", "林学院", "水产大学"],
    "民族类": ["民族大学", "民族学院"],
    "医药类": ["医学院", "军医", "卫生健康", "健康科学"],
    "地矿类": ["地质大学", "矿业大学", "石油大学", "石油学院", "矿业学院", "地质学院"],
}

# 考生意向专业 → 应排除的院校专业类型（意向院校豁免，见 apply_major_type_filter）
_MAJOR_EXCLUDED_SPECIALTIES: dict[str, list[str]] = {
    # IT/工科/理学：排除明显无工科的专业性院校（含医药类/地矿类/师范类，BUG-016补录）
    "计算机":   ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "软件工程": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "人工智能": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "数据科学": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "电子信息": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "通信工程": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "电气工程": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "自动化":   ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "机械工程": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "农林类", "师范类", "地矿类"],
    "土木工程": ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "地矿类"],
    "建筑学":   ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类", "地矿类"],
    "数学":     ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类"],
    "物理学":   ["艺术类", "体育类", "政法类", "中医药类", "医药类", "外语类"],
    "化学":     ["艺术类", "体育类", "政法类", "中医药类", "外语类"],
    "生物学":   ["艺术类", "体育类", "政法类", "中医药类", "外语类"],
    "环境工程": ["艺术类", "体育类", "政法类", "中医药类", "外语类"],
    # 医学类：排除明显无医学的专业性院校
    "临床医学": ["艺术类", "体育类", "政法类", "外语类", "师范类", "财经类", "民族类"],
    "口腔医学": ["艺术类", "体育类", "政法类", "外语类", "师范类", "财经类", "民族类"],
    "药学":     ["艺术类", "体育类", "政法类", "外语类", "师范类", "财经类"],
    "护理学":   ["艺术类", "体育类", "政法类", "外语类", "师范类", "财经类"],
    # 法学：主要排除艺体和纯理工/医学
    "法学":     ["艺术类", "体育类", "中医药类"],
    # 经管类
    "金融学":   ["艺术类", "体育类", "中医药类"],
...
# 白名单：即使院校名称匹配专业类型排除规则，以下院校仍强制保留（BUG-016补录）
# 格式：{ 考生意向专业 → [豁免院校名称列表] }
_MAJOR_SCHOOL_WHITELIST: dict[str, list[str]] = {
...
class SchoolRecord:
    ...
    major_match_label: Optional[str] = None  # BUG-019：专业意向匹配标注
    weighted_min_score: Optional[float] = None  # TASK-15：加权最低录取分（用于超高分段分差校验）
...
# 校名→城市精确覆盖表：用于正则提取结果有误的学校（优先于正则）
_SCHOOL_CITY_OVERRIDES: dict[str, str] = {
    # 校名前缀为地名但校区不在该地
    "中山大学": "广州",
    "中山大学南方学院": "广州",
    "中山大学新华学院": "东莞",
    # 广东省内非省会院校（无城市前缀，省会回退"广州"有误）
    "五邑大学": "江门",
    "岭南师范学院": "湛江",
    "广东海洋大学": "湛江",
    "广东医科大学": "湛江",
    ...
    "湖北师范大学": "黄石",
    "湖北汽车工业学院": "十堰",
    "湖北文理学院": "襄阳",      # BUG-020：实际在襄阳，非武汉
    "湖北民族大学": "恩施",      # BUG-020：实际在恩施，非武汉
    "三峡大学": "宜昌",
    "长江大学": "荆州",
    "湖北中医药大学": "武汉",    # 确认在武汉
    "江汉大学": "武汉",          # 确认在武汉
    ...
    "西南科技大学": "绵阳",
    "四川文化艺术学院": "绵阳",
    # 山东省内非省会院校（"中国XX大学"无城市前缀，省会回退"济南"有误）
    "中国石油大学（华东）": "青岛",
    "中国海洋大学": "青岛",
    # 江苏省内非省会院校
    "中国矿业大学": "徐州",
...
_CITY_RE = _re.compile(
    ...
    r"|宜昌|荆州|黄石|襄阳|十堰|孝感|黄冈|荆门|咸宁|鄂州|随州|恩施"
...
def _city_from_school(name: str, province: str) -> str:
    """从学校名称提取城市，提取失败时用省会兜底。优先查精确覆盖表。"""
    if name in _SCHOOL_CITY_OVERRIDES:
        return _SCHOOL_CITY_OVERRIDES[name]
...
def _extract_city(name: str) -> str:
    # Step 1: Military schools
    for keyword, city in _MILITARY_SCHOOL_CITY.items():
        if keyword in name:
            return city
    # Step 2: Independent college pattern — e.g. "北京科技大学天津学院" → "天津"
    indie = _re.match(r'.+大学(.+)学院$', name)
    if indie:
        m2 = _CITY_RE.match(indie.group(1))
        if m2:
            return m2.group(0)
    # Step 3: City prefix regex
...
def _calc_weighted_min_score(history: list[dict]) -> Optional[float]:
    """基于录取历史计算加权最低录取分，用于超高分段保底分差校验（TASK-15）。
    权重规则：3年 2025×0.5+2024×0.3+2023×0.2；2年 ×0.65+×0.35；1年 100%；无数据 None。
    """
    if not history:
        return None
    # 去重：每年只取最高 min_score（最宽松批次），过滤无效值
    year_map: dict[int, float] = {}
    for y in history:
        yr = y.get("year")
        ms = y.get("min_score")
        if yr and ms and ms > 0:
            if yr not in year_map or ms > year_map[yr]:
                year_map[yr] = float(ms)
    if not year_map:
        return None
    sorted_years = sorted(year_map.keys(), reverse=True)[:3]
    scores = [year_map[yr] for yr in sorted_years]
    if len(scores) == 1:
        return scores[0]
    if len(scores) == 2:
...
def _ultra_high_score_bottom_ok(s: SchoolRecord, student_score: int) -> bool:
    """TASK-15：超高分段（670+）保底院校分差校验。
    返回 True 表示院校通过校验（可保留），False 表示应剔除。
    """
    if student_score < ULTRA_HIGH_SCORE_THRESHOLD:
        return True  # 非超高分段，不拦截
    weighted_min = s.weighted_min_score
    if weighted_min is None:
        return True  # 无历史数据不拦截
    if student_score >= ULTRA_HIGH_SCORE_FLOOR and weighted_min < ULTRA_HIGH_HARD_MIN_SCORE:
        return False  # 硬拦截：680+分考生，院校历史最低分<600
    if student_score - weighted_min > ULTRA_HIGH_BOTTOM_GAP:
        return False  # 软拦截：分差超过50分
    return True


def apply_quality_threshold_filter(
    schools: list[SchoolRecord], score_segment: str, student_score: int = 0
) -> list[SchoolRecord]:
    """按 PRD §4.8 院校质量底线规则过滤候选学校。

    - 意向学校（is_intended）和意向城市（is_intended_city）标记学校不过滤
    - 低分段/未知段位不过滤（低分段保底可纳入专科）
    - 高分段保底档追加超高分段分差校验（TASK-15）
    """
        ...
        if score_segment == "high":
            if tier == 0:    # 冲刺档：985/211/双一流，或邻省省重点公办一本
                ...
                if not _is_vocational(s):
                    result.append(s)
            elif tier == 2:  # 保底档：排除专科/职业技术 + 超高分段分差校验
                if not _is_vocational(s) and _ultra_high_score_bottom_ok(s, student_score):
                    result.append(s)
            else:
                result.append(s)
    ...
    # TASK-15: 超高分段保底被全部剔除时，放宽分差阈值重新过滤，避免保底为空
    if score_segment == "high" and student_score >= ULTRA_HIGH_SCORE_THRESHOLD:
        bottom_count = sum(1 for s in result if (s.tier == 2 and not s.is_intended))
        if bottom_count < 2:
            # 放宽：将已剔除的保底候选（原本通过非分差过滤条件）重新尝试纳入
            relaxed_gap = ULTRA_HIGH_BOTTOM_GAP + 20  # 放宽20分
            for s in schools:
                if s.is_intended or s.school_id in {r.school_id for r in result}:
                    continue
                if s.tier != 2:
                    continue
                if _is_vocational(s):
                    continue
                weighted_min = s.weighted_min_score
                if weighted_min is None or (student_score - weighted_min) <= relaxed_gap:
                    result.append(s)
                    bottom_count += 1
                    if bottom_count >= 2:
                        break
...
# ──────────────────────────────────────────────────────────────────────────────
# PHASE 3.6: 专业意向过滤（BUG-019）
# ──────────────────────────────────────────────────────────────────────────────

def _classify_school_specialty(school: SchoolRecord) -> Optional[str]:
    """用院校名称关键词判断院校专业类型。综合类/985/211综合大学返回 None。"""
    name = school.name
    for sp_type, keywords in _SPECIALTY_SCHOOL_MARKERS.items():
        for kw in keywords:
            if kw in name:
                return sp_type
    return None
...
def _get_excluded_specialties(major_prefs: list[str]) -> set[str]:
    """汇总考生所有意向专业对应的排除类型集合。"""
    excluded: set[str] = set()
    for major in major_prefs:
        excluded.update(_MAJOR_EXCLUDED_SPECIALTIES.get(major, []))
    return excluded


def apply_major_type_filter(
    schools: list[SchoolRecord],
    major_prefs: list[str],
    intended_ids: set[int],
) -> list[SchoolRecord]:
    """按专业意向过滤推荐池（PRD §4.9）。
    意向院校（intended_ids）豁免过滤，但会被标记 major_match_label 警告。
    白名单院校（_MAJOR_SCHOOL_WHITELIST）无条件通过，不受专业类型排除规则影响。
    """
    if not major_prefs:
        return schools

    excluded = _get_excluded_specialties(major_prefs)
    if not excluded:
        return schools
    ...
    for major in major_prefs:
        whitelisted_names.update(_MAJOR_SCHOOL_WHITELIST.get(major, []))

    result = []
    for s in schools:
        # 白名单：强制通过，不受专业类型排除规则约束
        if s.name in whitelisted_names:
            result.append(s)
            continue
        sp_type = _classify_school_specialty(s)
        if sp_type and sp_type in excluded:
            if s.school_id in intended_ids:
                # 意向院校豁免但加注警告
                s.major_match_label = "⚠️ 该校暂无相关专业，请确认报考意向"
                result.append(s)
            # else: 从推荐池移除，不加入 result
        else:
            result.append(s)
    return result
...
async def generate_recommendation(req: RecommendRequest, db: AsyncSession) -> dict:
    ...
    for s in candidates:
        ...
        s.weighted_min_score = _calc_weighted_min_score(history)  # TASK-15
        s.data_quality = await detect_data_gaps(s.school_id, req.province, history, db)
        s.admission_data = _build_admission_summary(history)
        scored.append(s)

    # PHASE 3.5: 院校质量底线过滤（PRD §4.8）— 在 sort_and_slice 之前
    scored = apply_quality_threshold_filter(scored, score_segment, req.score)

    # PHASE 3.6: 专业意向过滤（BUG-019，PRD §4.9）
    if req.major_preference:
        intended_ids = {s.school_id for s in special_attention}
        scored = apply_major_type_filter(scored, req.major_preference, intended_ids)
        # 为特别关注区学校设置专业匹配警告标注
        excluded_sp = _get_excluded_specialties(req.major_preference)
        for s in special_attention:
            sp_type = _classify_school_specialty(s)
            if sp_type and sp_type in excluded_sp:
                s.major_match_label = "⚠️ 该校暂无相关专业，请确认报考意向"
...
def _school_to_dict(s: SchoolRecord) -> dict:
    ...
    if s.major_match_label:
        d["major_match_label"] = s.major_match_label
    if s.globe_expanded:
