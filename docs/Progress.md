...
| 更新日期 | 2026-06-23 (docs: TASK-02/TASK-03 状态确认并更新 Tasks.md，两任务均在 v5.19-v5.20 已完成) |
...
| **BUG-019 专业意向匹配核心缺陷** | **v5.20** | **P0** | **新增9类专业院校名称关键词映射+26专业排除规则；Phase 3.6过滤层（apply_major_type_filter）；意向院校豁免+警告标注；修复舞蹈/体育/中医药/政法/林业等院校混入CS推荐，政法/外语/师范/财经等混入医学推荐** | ✅ |
| **TASK-05 PC全宽基础框架+方案C主题迁移** | **v5.21** | **P1** | **废弃375px手机模拟框→max-width:1280px全宽；内容区max-width:1000px；全局CSS变量切换方案C浅色主题（bg:#F5F7FA/card:#FFFFFF/accent:#3B82F6等）；字号全局+1档(基础16px/标题24px/卡片18px)；输入框44px/主按钮48px；导航栏sticky；Toast改position:fixed** | ✅ |
...
| **TASK-13 管理后台话术管理模块** | **v5.x** | **P1** | **admin.html已有📝话术管理Tab含完整loadScripts()/新增/编辑/删除/排序功能（复用已有代码）** | ✅ |
...
| **v5.12** | **2026-06-19** | **高分考生冲刺0所/保底全职业四层根因修复（prior过高/L4门槛/globe_expanded过滤/segment兜底缺失）** |
| **v5.13** | **2026-06-19** | **Backfill Tier归属错误+职业院校漏检+一次性链接"无效链接"三项Bug修复** |
| **v5.14** | **2026-06-22** | **管理端PDF重下载功能：report_snapshots表持久化推荐快照；管理后台订单列表新增"⬇ 下载PDF"按钮；/?admin_preview=<order_id>在index.html重载报告重新下载PDF；学生链接端/s/recommend自动保存快照** |
