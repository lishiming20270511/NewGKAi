...
| **v5.9** | **2026-06-19** | **冲刺院校超5所Bug修复：根因=`sort_and_slice`的`_take`函数从tier0填充稳妥/保底槽位时未重新打标签，导致候选池全为tier0时15所全展示为"冲刺"；修复=给`_take`加`relabel_as`参数，借用其他tier学校时同步更新`s.tier`/`s.tier_label`，硬上限5所冲刺；fallback也按槽位序号强制打目标tier标签** |
